...
class Front {
	...
	public function load_assets() {
		...
		wp_register_script(
			'google-fonts', PBH_PLUGIN_URL . '/assets/libs/google-fonts/webfont.js',
			false,
			PBH()->config['cache_time'],
			true
		);
		wp_register_script(
			'page-builder-hub-front',
			PBH_PLUGIN_URL . '/assets/js/app.min.js',
			array(
				'jquery',
				'google-fonts'
			), PBH()->config['cache_time'], true
		);

		$js_vars = array(
			'ajaxurl'    => esc_url( admin_url( 'admin-ajax.php' ) ),
			'assetsPath' => PBH_PLUGIN_URL . '/assets',
		);

		wp_enqueue_script( 'page-builder-hub-front' );
		wp_localize_script( 'page-builder-hub-front', 'themeJsVars', $js_vars );
		...
		if ( $this->antispam_enabled() === 1 ) {
			wp_enqueue_script(
				'page-builder-hub-antispam',
				PBH_PLUGIN_URL . '/assets/js/antispam.js',
				array(
					'jquery',
				),
				PBH()->config['cache_time'], true
		...
		// CSS styles
		wp_enqueue_style(
			'page-builder-hub-libs',
			PBH_PLUGIN_URL . '/assets/css/libs/libs.css', false,
			PBH()->config['cache_time']
		);
		wp_enqueue_style(
			'page-builder-hub-style',
			PBH_PLUGIN_URL . '/assets/css/front/front.css',
	...
	public function add_gtm_head() {
		...
		if ( ! empty( $tag_manager_code ) && strpos( $site_url, 'wpengine.com' ) === false ) {
			...
			PBH()->View->load( '/template-parts/gtm',
